...
const renderCalendar = () => {
    ...
    const selectedDateHeader = document.querySelector(".tile h2"); // Select the h2 tag for the selected date

    dayElements.forEach(day => {
        day.addEventListener("click", function() {
            if (!this.classList.contains("inactive")) {
                const selectedDate = `${months[currMonth]} ${currYear}, ${this.textContent}`;

                // Set the content of the h2 tag to the selected date
                selectedDateHeader.textContent = `Date: ${selectedDate}`;

                // Update other elements or perform necessary actions with the selected date
            }
        });
    });
}
